...
g_T_an = Expression('g_n_cx*a_an', degree=1, g_n_cx=g_n_cx, a_an=a_an)
...
#import numpy
#
#print "#x n T Z D_an g_n_an g_n_cx g_T_an g_T_cx g_Z_an g_Z_cx g_OL f_OL g_n_bulk g_T_bulk g_Z_bulk"
#for j in numpy.arange(0.0, 1.0, 0.01):
#	for k in numpy.arange(-3.0, 3.0, 0.01):
#		print j, n(j), T(j), k, D_an(j), g_n_an(j), g_n_cx(j), g_T_an(j), g_T_cx(j), g_Z_an(j), g_Z_cx(j), g_OL(j), f_OL(j,k), g_n_bulk(j,k), g_T_bulk(j,k), g_Z_bulk(j,k)
