...
n_0  = symfun( 4e17*a_in0*(Temp(x) / 100)^(3/4), x );
...
g_n_an = vpa(symfun( -charge* n(x) * D_an(x), x ),3)

g_n_cx = vpa(symfun( -m_i*n_0(x)*R_cx * n(x)*Temp(x) / B_p^2, x ),3)

g_n_bulk = vpa(symfun( aspect^2*sqrt(pi) / (8*a_m) * n(x) * m_i * rho_pi(x) * (v_Ti(x))^2 * B_p * xi_p(x, Z), [x Z] ),3)

g_n_sum = symfun( g_n_an(x) + g_n_cx(x) + g_n_bulk(x, Z), [x Z] );

%% ---------------- g_T coefficients ------------------------------

g_T_an = vpa(symfun( g_n_cx * a_an, x ),3)

g_T_cx = vpa(symfun( a_cx * g_n_cx, x ),3)

g_T_bulk = vpa(symfun( aspect^2*sqrt(pi) / (8*a_m) * n(x) * m_i * rho_pi(x) * (v_Ti(x))^2 * (B_p*xi_p(x, Z) - B*xi_t(x, Z)), [x Z] ),3)

g_T_sum = symfun( g_T_an(x) + g_T_cx(x) + g_T_bulk(x, Z), [x Z] );

%% ---------------- g_Z coefficients ------------------------------

g_Z_an = vpa(symfun( -charge * n(x) * D_an(x) / rho_pi(x), x ),3)

g_Z_cx = vpa(symfun( m_i*n_0(x)*R_cx * n(x)*Temp(x) / (rho_pi(x) * B_p^2), x ),3)

g_Z_bulk = vpa(symfun( aspect^2*sqrt(pi) / (4*a_m) * n(x) * m_i * (v_Ti(x))^2 * B_p * xi_p(x, Z), [x Z] ),3)
...
f_OL = vpa(symfun( g_OL(x) * exp(-sqrt(nu_ai(x) + Z^4)) / sqrt(nu_ai(x) + Z^4), [x Z] ),3)
