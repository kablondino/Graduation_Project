...
+ What is the bifurcation structure of the model proposed by Zohm?
...
+ Two separate fold bifurcations are necessary to describe the hysteresis, since the heating to trigger the L-H transition is different than for H-L transition.
...
+ Continuity equation for mass (density) and energy, with a single temperature and all the particle and energy deposition into the plasma is somewhere in the core outside of the model:
...
+ The cusp condition is defined as $\mathbf{u}_1^T (M_2 \mathbf{v}_1) \mathbf{v}_1 \,=\, 0$, which is taken care of by differentiating the 3-tensor $M_2 \,=\, \dfrac{\partial M_1}{\partial \mathbf{v_0}}$. This leads to a similar condition:
...
+ The considered model assumes L-mode radial transport is dominated by turbulence. It is also assumed that shear in the $\mathbf{E}\times\mathbf{B}$-flow is capable of tearing apart turbulent eddies.
	...
	+ This means it is necessary to include the evolution of the radial electric field and corresponding flow profile. In this model, there is not small-scale tearing and possible back-reaction of turbulence-generating zonal flows.
...
_NOTE:_ This section is very similar to 4.4, and is only more specific with $\mathbf{E}\times\mathbf{B}$-flow.
...
+ Figure 5.6 shows the typical evolution of the solution, as $\theta_1 \rightarrow \theta_2 \rightarrow \theta_3 \rightarrow \theta_4$, with the subfigures showing the state. Read the caption of the figure for more info.
	...
	+ The thin-barrier width is of the order of the viscosity, _e.g._ several gyro-radii.
...
<!-- ------------ THE ABSTRACT ------------
	...
	The analytic bifurcation analysis characterises the parameter space structure of the transition dynamics.
...
+ How can we identify the co-dimension 3 bifurcation in 1-D models?
	...
	2. Two eigenvectors with vanishing eigenvalues of the linear operator of the system need to be found for the fold bifs. At the cusp bif, the second term of the Taylor expansion evaluated in the same direction as the fold bif must vanish.
...
+ How do these 1-D bifurcating models combine transitions in time and space?
	...
	+ Investigating the system (a general nonlinear PDE, first-order deriv. in time and second-order deriv. in space, non-monotonic, with a control parameter, Eq. 5-14) lead to the formulation of a generalized equal area rule for simultaneous transitions in space and time.
...
+ It is well known that at low densities, the scaling laws start to fail. Instead of a decreasing power threshold for decreasing density, it begins to increase again below a certain density. This is called the "roll over of the power threshold."
